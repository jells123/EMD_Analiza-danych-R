...
data <- data[-1] # skip 'X' index column
all_rows_number <- nrow(data)
str(data)
```
Z rezultatu wywołania funkcji `str` wynika, że niektóre kolumny zawierają znaki zapytania, co oznacza brakujące dane. Są to zmiene dotyczące dostępności planktonu i temperatury przy powierzchni wody.
...
min_ratio = nrow(data.frame(x=cfin_eq_min)) / nrow(numeric_data)
...
corrplot(cor(data))
```
>>>>>>> 1cbae5090d9abdbeecdc42410329d1d01a84f4a5

## Zmiana rozmiaru śledzi w czasie <a name="rozmiar"></a>
```{r}
p <- plot_ly(data[sample(nrow(data), 1000), 1:2], x = any, y = ~length, type = 'scatter', mode = "markers")
p
```
